...
Q = diag([0.02,0.02,0.05,1,1,1]); % State penalty
...
%     [Pc, qc, Sc] = constraint_mats(F,G,Pu,qu,Px,qx,Pxf,qxf); % Constraints As Linear Inequality 
    ...
    %Control    
    [Useq,Cst] = quadprog(H,L*X(:,k)); % ,Pc,qc + Sc*X(:,k) Solve Quadratic program
    U(:,k) =  Useq(1:3);
    Cost(:,k) = Cst;
...
for k = 2:kT-1
    ...
    %Control    
    [c,Cst] = quadprog(Sc1,Scx*X(:,k)); % Pc,qc + Sc*X(:,k) Solve Quadratic program
    U(:,k) =  -Kdt*X(:,k) + c(1:3);
    Cost(:,k) = Cst;
